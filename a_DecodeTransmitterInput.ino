...
void initTxWatchdog() {
  ...
  Timer1.initialize(kTxSignalPeriod);
  Timer1.attachInterrupt(TxSignalCheckISR, kTxSignalPeriod);
}
...
void processTxSignal(unsigned int *txSignal) {
    ...
    txSignal[0] = (constrain(rawInRoll, kState0Min, kState0Max) - kState0Min) / 4;
    txSignal[1] = (constrain(rawInPitch, kState0Min, kState0Max) - kState0Min) / 4;
    txSignal[2] = (constrain(rawInThrottle, kState0Min, kState0Max) - kState0Min) / 4;
    txSignal[3] = (constrain(rawInYaw, kState0Min, kState0Max) - kState0Min) / 4;

    // Dead zone
    
    txSignal[0] = deadZone(txSignal[0],90,110,100);
    txSignal[1] = deadZone(txSignal[1],90,110,100);
    txSignal[2] = deadZone(txSignal[2],0,40,0);
    txSignal[3] = deadZone(txSignal[3],95,110,100);
...
unsigned int deadZone(int inputNo, int bandLow, int bandHigh, int DZVal){
  return inputNo<bandHigh && inputNo>bandLow ? DZVal : inputNo;
}
