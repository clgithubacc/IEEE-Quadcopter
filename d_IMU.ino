 ...
 *  Due to some issues with yaw output, the original IMU code
 *  and MPU9250 library files are merged into one single file.
 *  Since variables here can also be accessed from other fun-
 *  ctions, myIMU will no longer be used.
 *  
 *  The reason that causes incorrect yaw output is still not
 *  clear. Seems that it relates to frequently assign/get
 *  values to variables in myIMU. To see a list of excluded
 *  factor, please check VSTS sprint 1.
 *  
 *  The following code is based on Shubham Paul's modified
 *  version of Kris Winer's MPU9250 code and the original
 *  IMU code.
 */
 #include <SPI.h>
 #include <Wire.h>
 #include "IMURegisterDefinitions.h"
